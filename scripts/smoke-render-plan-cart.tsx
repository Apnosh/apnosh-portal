 ...
 *   2. the PLAN VIEW renders one full cart card per item (title, version line + its
 *      price, each add-on line + its PDP-exact price, per-item summary, Edit/Remove),
 *      the price-details breakdown (one-time + monthly from planTotals), the running
 *      total, the honesty line, and the Check out button
...
async function main() {
  ...
  const { serviceById, cadenceOf, plainNameOf } = await import('../src/lib/campaigns/catalog')
  ...
  // The team lane string exactly as the gbp card authors it (GBP_DOER_APNOSH).
  const GBP_TEAM = `done for you by Apnosh, $${(ITEM_PRICES.gbp && ITEM_PRICES.gbp.oneTime) || 365}`
  const cart = [
    { itemId: 'gbp', doer: GBP_TEAM, options: ['gbp-posts'], addedAt: 1 },
  ...
  ok(bar.includes('>2<!-- -->') || / 2 /.test(bar.replace(/<!-- -->/g, ' ')), 'item count (2) renders')
  ok(bar.includes(`From $${t.oneTime.toLocaleString()} + $${t.perMonth.toLocaleString()}/mo`), `the PDP-exact total renders (From $${t.oneTime} + $${t.perMonth}/mo)`)
  ...
  console.log('\n== 2) the plan view (full cart cards + price details) ==')
  ...
  ok(view.includes('Polish your Google profile') && view.includes('A short video'), 'one card per item, real card titles')
  // (a) the chosen version renders as a labeled, priced line (team lane on gbp)
  ok(view.includes('Apnosh does it'), 'the version label renders on the versioned (gbp) card')
  ok(view.includes(`$${((ITEM_PRICES.gbp && ITEM_PRICES.gbp.oneTime) || 365).toLocaleString()}`), 'the version price renders next to the version label')
  // (b) the add-on renders with the PDP's exact name + price delta
  const gbpPosts = serviceById('gbp-posts')
  if (!gbpPosts) throw new Error('gbp-posts missing from the catalog')
  const { price: optPrice, cadence: optCad } = cadenceOf(gbpPosts)
  const optDelta = optCad.kind === 'recurring' ? `+$${optPrice.toLocaleString()}/mo` : `+$${optPrice.toLocaleString()}`
  ok(view.includes(plainNameOf(gbpPosts)), `the add-on renders by its plain name (${plainNameOf(gbpPosts)})`)
  ok(view.includes(optDelta), `the add-on price renders exactly as the PDP shows it (${optDelta})`)
  // per-item summary still prices in the PDP idiom
  const mReel = planItemMoney(cart[1])
  ok(view.includes(`From $${mReel.oneTime.toLocaleString()}`), `the creative card prices in the PDP idiom (From $${mReel.oneTime})`)
  ok(view.includes('Edit') && view.includes('Remove'), 'Edit and Remove affordances render on the cards')
  // (c) the price-details breakdown: one-time and monthly figures from planTotals
  ok(view.includes('Price details'), 'the price-details card renders')
  ok(view.includes('One-time work') && view.includes(`$${t.oneTime.toLocaleString()}`), `the one-time figure renders ($${t.oneTime})`)
  ok(view.includes('Monthly services') && view.includes(`$${t.perMonth.toLocaleString()}/mo`), `the monthly figure renders ($${t.perMonth}/mo)`)
  ok(view.includes('Billed monthly once your services start.'), 'the honest monthly sub-line renders')
  ok(view.includes(`From $${t.oneTime.toLocaleString()} + $${t.perMonth.toLocaleString()}/mo`), 'the running total renders')
