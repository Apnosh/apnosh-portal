...
export interface JobSpec {
  ...
  emoji: string
  /** how the piece is built. 'single' = one page; 'carousel' = really wants
   *  multiple pages (a story cannot live on one image); 'either' = genuinely
   *  works both ways, the owner decides on the build step with no push. */
  format?: 'single' | 'either' | 'carousel'
  ...
  /** text renditions the same answers can become (About section, GBP
   *  description, team bio). Offered on the build step, priced per piece. */
  ...
  voice?: {
    ...
    questions?: readonly { label: string; ph: string; long?: boolean }[]
    ...
    angles?: readonly { id: string; label: string; sub: string; questions: readonly { label: string; ph: string; long?: boolean }[] }[]
  }
...
const spec = (
  ...
  extra: Partial<Pick<JobSpec, 'headline' | 'asks' | 'timing' | 'scope' | 'voice' | 'format' | 'places' | 'written'>> = {},
