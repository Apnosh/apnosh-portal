...
import {
  ...
  referralBlock, type ReferralStatus,
} from './model'
 ...
 *
 * WHY THAT IS SAFE. An intent that never collects is not a spent credit. A later checkout may
 * RE-CLAIM a credit whose held intent has no collected payment row — so an abandoned cart returns
 * the money to the owner on their next try, without a Stripe call and without a cron.
 *
 * The claim is written with the previous holder in the WHERE clause, so two checkouts racing for
 * the same credit cannot both win it.
 *
...
export async function claimFriendCredit(clientId: string, intentKey: string, maxCents: number): Promise<ClaimedCredit | null> {
  ...
  try {
    ...
    for (const row of rows) {
      // A hold is STALE when the checkout holding it never collected. Its cents go back into what
      // is available; anything spent on a checkout that DID collect stays spent.
      const heldByAnIntent = !!row.consumed_at && !!row.consumed_intent_id
      const stale = heldByAnIntent && !(await intentCollected(row.consumed_intent_id as string))
      const settled = Math.max(0, (row.consumed_cents || 0) - (stale ? (row.held_cents || 0) : 0))
      const use = Math.min(Math.max(0, (row.cents || 0) - settled), Math.round(maxCents))
      if (use <= 0) continue
      let q = admin.from('client_credits')
        ...
        .eq('id', row.id)
      // Optimistic lock on the previous holder: whoever read it first is who may take it, so two
      // tabs at the same checkout cannot both spend the same credit.
...
/** Did the checkout that holds a credit actually take money? Reads the ledger, never Stripe. */
async function intentCollected(intentId: string): Promise<boolean> {
  try {
    ...
    // Unreadable → treat the hold as real. Failing this way loses an owner a $50 credit at worst;
    // failing the other way spends the same $50 twice.
    return true
  }
}
 ...
 * one. Until this runs the hold is keyed to nothing, which reads as "never collected" — so a
 * checkout that dies between the claim and the intent gives the credit back on the next try.
 */
export async function stampCreditIntent(creditId: string, intentId: string): Promise<void> {
  if (!referralsEnabled() || !creditId || !intentId) return
  try {
    await createAdminClient().from('client_credits').update({ consumed_intent_id: intentId }).eq('id', creditId)
  } catch (e) { warn('could not tie the credit to the checkout', e) }
}
 ...
 */
export async function releaseFriendCredit(creditId: string): Promise<void> {
  if (!referralsEnabled() || !creditId) return
  try {
    const admin = createAdminClient()
    const { data } = await admin.from('client_credits').select('consumed_cents, held_cents').eq('id', creditId).maybeSingle()
    const settled = Math.max(0, ((data?.consumed_cents as number) || 0) - ((data?.held_cents as number) || 0))
    await admin.from('client_credits')
      .update({ consumed_cents: settled, held_cents: 0, consumed_at: settled > 0 ? new Date().toISOString() : null, consumed_intent_id: null })
      .eq('id', creditId)
