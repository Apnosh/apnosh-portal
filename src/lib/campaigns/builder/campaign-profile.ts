...
export interface CampaignProfile {
  ...
  /** Setup serviceIds the restaurant ALREADY has in place (Google profile connected, socials linked),
   *  so the go-live estimate skips them — we never re-quote setup that's already done. */
  doneSetup: string[]
  /** Days the owner marked SLOW in onboarding (full names, e.g. ['Monday','Tuesday']) — pre-fills
   *  the slow-nights {days} picker and feeds the brain's slowNights signal. */
  slowDays: string[]
}

const EMPTY: CampaignProfile = {
  neighborhood: null, city: null, cuisine: null, concept: null, targetAudience: null,
  monthlyBudget: null, rating: null, ratingCount: null, topSegment: null, primaryGoal: null, currentSpecial: null, presence: null, doneSetup: [], slowDays: [],
}

export async function getCampaignProfile(clientId: string): Promise<CampaignProfile> {
  try {
    const admin = createAdminClient()
    const [bizRes, signals, specialRes, chanRes, socialRes] = await Promise.all([
      ...
      admin.from('businesses')
        .select('city, cuisine, cuisine_other, target_audience, target_location, restaurant_subtype, monthly_budget, primary_goal, goal_detail, slow_periods')
      ...
      // Live connections → what's already set up, so the go-live estimate skips done foundations.
    ...
    // Map live connections → setup serviceIds already satisfied. GBP connected ⇒ the Google profile is
    // claimed + verified (gbp-setup, review-claim); socials linked ⇒ channel-connect. Conservative on
    // purpose — only foundations we can prove are in place.
    const activeChannels = ((chanRes?.data ?? []) as { channel?: string }[]).map((c) => c.channel)
    const activeSocials = ((socialRes?.data ?? []) as { platform?: string }[]).map((s) => s.platform)
    const doneSetup: string[] = []
    if (activeChannels.includes('google_business_profile')) doneSetup.push('gbp-setup', 'review-claim')
    if (activeSocials.includes('instagram') || activeSocials.includes('facebook')) doneSetup.push('channel-connect')
    return {
      ...
      doneSetup,
      // onboarding rhythm: { Mon: 'busy'|'steady'|'slow', ... } -> the SLOW days, as full names
      slowDays: (() => {
        const FULL: Record<string, string> = { Mon: 'Monday', Tue: 'Tuesday', Wed: 'Wednesday', Thu: 'Thursday', Fri: 'Friday', Sat: 'Saturday', Sun: 'Sunday' }
        const sp = b.slow_periods
        if (!sp || typeof sp !== 'object' || Array.isArray(sp)) return []
        return Object.entries(sp as Record<string, string>).filter(([, v]) => v === 'slow').map(([k]) => FULL[k]).filter((d): d is string => !!d)
      })(),
    }
  } catch {
    return EMPTY
