...
export async function checkClientAccess(clientId: string): Promise<AccessCheckResult> {
  ...
  // Path 1 + 2: staff role or direct client linkage
  ...
  // Paths 3 + 4 require admin client to bypass RLS
  ...
  const { data: business } = await admin
    ...
    .maybeSingle()
  if (business) return { authorized: true, userId: user.id }

  // Path 4: magic-link portal
  const { data: clientUser } = await admin
    ...
    .eq('client_id', clientId)
    .maybeSingle()
  if (clientUser) return { authorized: true, userId: user.id }
