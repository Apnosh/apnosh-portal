...
export function suggestItems(i: SuggestInput): ItemPick[] {
  ...
  if (ct) {
    ...
    g.on = ct !== 'none'; g.options = { ...g.options, count: 1, from: shootish ? 'shoot' : ct === 'licensed' ? 'stock' : 'own' }
    g.why = ct === 'shoot' ? 'The post graphic, from the day\'s photos' : ct === 'queue' ? 'The post graphic, once the shoot lands' : ct === 'licensed' ? 'Designed on a licensed photo in your style' : ct === 'none' ? 'No picture, so no graphic' : 'Designed from your photo'
    if (ct === 'shoot') { const pk = PACKAGES.standard; g.options = { ...g.options, count: 1, included: 1, level: pk.graphicLevel, incLevel: pk.graphicLevel }; v.on = true; v.options = { ...v.options, count: pk.reels, included: pk.reels, filmed: 'shoot' }; v.why = 'A Reel cut from the day, inside the package' }
...
/* ONE GRAPHIC (owner 2026-09-24): an announcement needs one post graphic (or one per dish, or one
   carousel with a slide per dish), never a stack of them. The levels change its quality instead: the
   full day makes it at The works. The two and four graphics the bigger days used to include are gone,
   and their price came off at the package rate each ($185), plus The works on the full day ($225):
   half a day $1,199 - 2 x $185 = $829; a full day $1,899 - 4 x $185 + $225 = $1,384. */
export const PACKAGES: Record<PackageTier, { photos: number; graphics: number; graphicLevel: 2 | 3; reels: number; cents: number; label: string }> = {
  standard: { photos: 15, graphics: 1, graphicLevel: 2, reels: 1, cents: 69900, label: 'a quick visit' },
  full: { photos: 25, graphics: 1, graphicLevel: 2, reels: 2, cents: 82900, label: 'half a day' },
  works: { photos: 40, graphics: 1, graphicLevel: 3, reels: 4, cents: 138400, label: 'a full day' },
}
...
export function ladderFor(i: SuggestInput, items: ItemPick[]): Ladder {
  ...
  if (ct === 'shoot') { const pk = PACKAGES.standard; simple.photos = { photos: pk.photos, tier: 'standard' }; simple.graphic = { count: 1, included: 1, level: pk.graphicLevel, incLevel: pk.graphicLevel }; simple.video = { count: pk.reels, included: pk.reels, filmed: 'shoot', style: 'dish', captions: true } }
  ...
  if (ct === 'shoot') { const pk = PACKAGES.full; recommended.photos = { photos: pk.photos, tier: 'full' }; recommended.graphic = { count: 1, included: 1, level: pk.graphicLevel, incLevel: pk.graphicLevel }; recommended.video = { count: pk.reels, included: pk.reels, filmed: 'shoot', style: 'dish', captions: true } }
  else if (ct === 'queue') { recommended.graphic = { count: 1, included: 0 }; recommended.video = { count: 1, included: 0, filmed: 'shoot', style: 'dish', captions: true } }
  ...
  if (ct === 'shoot') { bigger.photos = { photos: PACKAGES.works.photos, tier: 'works' }; bigger.graphic = { count: 1, included: 1, level: PACKAGES.works.graphicLevel, incLevel: PACKAGES.works.graphicLevel } } else if (ct === 'queue') bigger.graphic = { count: 1, included: 0 }; else if (ct !== 'none') bigger.graphic = { count: 1 }
  ...
  const notes = {
    simple: ct === 'shoot' ? 'A quick visit: 15 photos, a Reel and the post graphic' : ct === 'queue' ? 'The post now, more when the shoot lands' : ct === 'licensed' ? 'A designed post and Google' : ct === 'none' ? 'Words on Google and Facebook. Free' : 'Your photo on your channels and Google. Free',
