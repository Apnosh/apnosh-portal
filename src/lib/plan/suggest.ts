...
export function suggestItems(i: SuggestInput): ItemPick[] {
  ...
  const gOn = !i.facts.hasMedia || isDeal || isEvent || isOpen
  add('graphic', gOn, !i.facts.hasMedia ? 'There is no photo, so the desk designs one' : isDeal || isEvent ? 'A deal needs the dates on the picture' : isOpen ? 'An opening needs the date and the address on it' : 'You have a photo. Add a graphic if you want the price on it', { where: ['post'], priceOn: !!i.facts.price, brandKit: true, spanish: false, from: i.facts.hasMedia ? 'own' : 'ours' }, i.prices.graphic)
  /* video: on when they already have clips, or when a creator visit makes it free of a trip */
  const vOn = i.facts.hasVideo
  add('video', vOn, i.facts.hasVideo ? 'You have video. A Reel reaches about twice what a photo does' : i.creator?.nearby ? `Add it and we film it when ${first} visits, no extra trip` : 'Reels reach about twice what a photo does. Ten seconds on your phone is enough', { filmed: i.facts.hasVideo ? 'clips' : i.creator?.nearby ? 'creator' : 'visit', style: 'dish', captions: true, tiktok: false, spanish: false }, i.prices.video)
  add('photos', isOpen, isOpen ? 'An opening needs the room and the team on camera' : 'A shoot day. Add it when you want photos of more than this', { list: [], reel: false }, i.prices.shoot)
  ...
  const cOk = !!i.creator?.nearby && (isDish || isDeal || isEvent || isOpen)
  add('creator', cOk && (i.budgetCents == null ? isOpen : true), i.creator?.nearby ? `${first} reaches about ${i.creator.nearby.toLocaleString()} people nearby and posts what you make` : 'No local creator with verified reach fits yet', i.creator ? { slug: i.creator.slug, tierName: i.creator.tierName ?? null, date: i.creator.date ?? null, code: true, repost: true, whitelist: false } : {}, i.creator?.fromCents ?? 0)
  add('print', isDeal, isDeal ? 'A table tent sells a deal at every table' : isOpen ? 'A window poster for the front' : 'A table tent or a window poster', { kind: isOpen ? 'poster' : 'tent' }, i.prices.print)
  ...
  add('taste', isDish, 'Free bites convert better than anything online. It goes on the team card', { days: 7 }, 0)
