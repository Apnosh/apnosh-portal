...
const KIND_WORD: Record<string, string> = { post: 'post', graphic: 'graphic', reel: 'Reel', photos: 'shoot day', boost: 'boost', print: 'print', creator: 'creator visit', offer: 'deal' }
...
export default function PlanMonthPage({ clientId, month: monthParam, historyHref, mode = 'plan' }: { clientId: string; month: string | null; /** where the campaigns that already ran live */ historyHref?: string; /** plan = Plan ahead (drafting); campaigns = the actual month: no lean, no estimates, fill / swap / push / scrap on slots, Order history */ mode?: 'plan' | 'campaigns' }) {
  ...
  const [view, setView] = useState<'month' | 'list' | 'money' | 'rhythm' | 'upcoming'>('month')
  ...
  return (
    <div className="cr" style={{ padding: '2px 16px 0', color: C.ink, maxWidth: 480, margin: '0 auto', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', gap: 4, padding: 3, borderRadius: 12, background: '#f2f2f5', marginTop: 12 }}>
        {(mode === 'campaigns' ? ([['month', 'Month'], ['upcoming', 'Upcoming'], ['list', 'List']] as const) : ([['month', 'Month'], ['list', 'List'], ['money', 'Order'], ['rhythm', 'Rhythm']] as const)).map(([k, l]) => <button key={k} type="button" onClick={() => setView(k)} style={{ flex: 1, font: 'inherit', fontSize: 13, fontWeight: 700, padding: '8px 0', borderRadius: 9, border: 0, background: view === k ? '#fff' : 'transparent', color: view === k ? C.ink : C.mute, boxShadow: view === k ? '0 1px 3px rgba(0,0,0,.10)' : 'none', cursor: 'pointer' }}>{l}</button>)}
      </div>
      ...
      {view === 'month' && (
        <div className="pm-in">
          ...
          {mode === 'campaigns' && (() => { const ideas = on.filter((x) => x.status === 'draft' && !x.date); return (
            <div style={{ marginTop: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={k2 as React.CSSProperties}>Ideas</span><span style={{ flex: 1 }} /><button type="button" onClick={() => setPencil({ date: null, kind: 'post', subject: '' })} style={{ font: 'inherit', fontSize: 12.5, fontWeight: 700, color: C.greenDk, border: 0, background: 'none', cursor: 'pointer', padding: 0, marginTop: 16 }}>+ Pencil in an idea</button></div>
              {ideas.length === 0 && <div style={{ fontSize: 12, color: C.mute, marginTop: 6 }}>Things you might do, with no day yet. Nothing here costs anything or goes to the team.</div>}
              {ideas.map((x) => <div key={x.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `0.5px solid ${C.line}`, fontSize: 13.5, fontWeight: 600 }}><span style={{ width: 30, height: 30, borderRadius: 9, border: `1.5px dashed ${HUE[x.stage]}`, display: 'grid', placeItems: 'center', flex: 'none', ['--c2' as string]: HUE[x.stage] }}><span style={{ width: 20, display: 'block', opacity: .6 }}><Drawing spec={{ scene: SCENE[x.kind] }} name="" rating="" t={(y) => y} /></span></span><button type="button" onClick={() => setPiece(x)} style={{ flex: 1, minWidth: 0, textAlign: 'left', font: 'inherit', border: 0, background: 'none', padding: 0, cursor: 'pointer', color: C.ink, fontWeight: 600 }}>{x.subject ?? x.label}<small style={{ display: 'block', fontWeight: 500, color: C.mute, fontSize: 11.5 }}>{KIND_WORD[x.kind] ?? x.kind} · draft</small></button><label style={{ fontSize: 12, fontWeight: 700, color: C.greenDk, cursor: 'pointer', position: 'relative' }}>Put it on a day<input type="date" min={new Date().toISOString().slice(0, 10)} onChange={(e) => { if (e.target.value) save({ action: 'update', slotId: x.id, date: e.target.value }, `date:${x.id}`) }} style={{ position: 'absolute', inset: 0, opacity: 0, cursor: 'pointer', width: '100%' }} /></label></div>)}
            </div>) })()}
      ...
      {view === 'upcoming' && (() => {
        ...
        const plus = (iso: string, n: number) => { const d = dt(iso); d.setDate(d.getDate() + n); return d.toISOString().slice(0, 10) }
        const nextWk = plus(wk, 7), after = plus(wk, 14)
        const up = extras.filter((e) => e.state !== 'done' && e.state !== 'stopped' && e.state !== 'draft' && (!e.date || e.date >= today))
        const secs: [string, Extra[]][] = ([['Needs you', up.filter((e) => e.state === 'needs')], ['This week', up.filter((e) => e.state !== 'needs' && e.date && e.date < nextWk)], ['Next week', up.filter((e) => e.state !== 'needs' && e.date && e.date >= nextWk && e.date < after)], ['Later', up.filter((e) => e.state !== 'needs' && e.date && e.date >= after)], ['All month', up.filter((e) => e.state !== 'needs' && !e.date)]] as [string, Extra[]][]).filter(([, xs]) => xs.length)
        const word = (e: Extra) => e.state === 'idea' ? ['draft', '#fff', C.mute] : e.state === 'needs' ? ['needs you', '#fff4e0', '#8a5a0c'] : e.state === 'team' || e.state === 'live' ? ['with the team', C.greenSoft, C.greenDk] : e.fill === 'open' ? ['open', '#fff', C.mute] : ['coming', '#f2f2f5', C.mute]
        const collapse = (xs: Extra[]) => { const out: (Extra & { n?: number; days?: string })[] = []; const byWeek = new Map<string, Extra[]>(); for (const e of xs) { if (e.kind === 'post' && e.group.kind === 'month' && e.date) { const k = (() => { const d = dt(e.date); d.setDate(d.getDate() - d.getDay()); return d.toISOString().slice(0, 10) })(); byWeek.set(k, [...(byWeek.get(k) ?? []), e]) } else out.push(e) } for (const [, ps] of byWeek) out.push({ ...ps[0], id: `posts:${ps[0].id}`, n: ps.length, days: ps.map((x) => WD[dt(x.date!).getDay()]).join(' · ') }); return out.sort((a, b) => (a.date ?? '9999').localeCompare(b.date ?? '9999')) }
        return (
          <div className="pm-in">
            {secs.length === 0 && <div style={{ marginTop: 18, border: `0.5px dashed ${C.line}`, borderRadius: 16, padding: '22px 16px', textAlign: 'center', fontSize: 14, fontWeight: 700 }}>Nothing coming up.</div>}
            {secs.map(([label, xs]) => <div key={label}><div style={k2}>{label}</div>{collapse(xs).map((e) => { const [w, bg, fg] = word(e); const dated = e.date && label !== 'All month'; const inner = <><span style={{ width: 30, flex: 'none', ['--c2' as string]: HUE[(KIND_STAGE as Record<string, Stage>)[e.kind] ?? 'action'] }}>{e.fill === 'open' ? <span style={{ width: 30, height: 30, borderRadius: 9, border: '1.5px dashed #c9c9d0', display: 'block', boxSizing: 'border-box' }} /> : <Drawing spec={{ scene: (SCENE as Record<string, Scene>)[e.kind] ?? 'else' }} name="" rating="" t={(x) => x} />}</span><span style={{ flex: 1, minWidth: 0 }}><span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.group.emoji ? `${e.group.emoji} ` : ''}{e.n && e.n > 1 ? `${e.n} posts` : e.label}</span><small style={{ display: 'block', fontWeight: 500, color: C.mute, fontSize: 11.5, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.n && e.n > 1 ? e.days : [dated ? niceDate(e.date!) : '', e.detail].filter(Boolean).join(' · ')}</small></span><span style={{ fontSize: 10.5, fontWeight: 800, padding: '3px 8px', borderRadius: 99, background: bg, color: fg, whiteSpace: 'nowrap', border: e.fill === 'open' || e.state === 'idea' ? '1.5px dashed #c9c9d0' : 0 }}>{w}</span></>; const style: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: `0.5px solid ${C.line}`, fontSize: 13.5, fontWeight: 600, color: C.ink, textDecoration: 'none', cursor: 'pointer' }; return e.source === 'slot' && e.date ? <button key={e.id} type="button" onClick={() => { if (e.month && e.month !== m.month) go(e.month); else setDay(e.date) }} style={{ font: 'inherit', ...style, width: '100%', textAlign: 'left', border: 0, background: 'none', borderBottom: `0.5px solid ${C.line}` }}>{inner}</button> : <a key={e.id} href={e.href ? (e.href.includes('?') ? `${e.href}&clientId=${clientId}` : `${e.href}?clientId=${clientId}`) : '#'} style={style}>{inner}</a> })}</div>)}
          </div>
        )
      })()}
      ...
      {day && (() => { const here = on.filter((x) => x.date === day && !allMonth(x)); const ev = eventsOn(day); const occ = data.season.find((s) => s.month === m.month)?.occasions.find((o) => o.date === day); return (
        <div role="dialog" aria-modal="true" onClick={() => setDay(null)} style={sheet}>
          <div className="cr pm-in" onClick={(e) => e.stopPropagation()} style={sheetIn}>
            ...
            {here.map((s) => {
              ...
              return (
                <div key={s.id ?? `${s.kind}:${s.date}`}>
                  {mode === 'campaigns' ? <button type="button" onClick={() => setPiece(s)} style={{ width: '100%', textAlign: 'left', font: 'inherit', border: 0, background: 'none', padding: 0, cursor: 'pointer', color: C.ink }}>{row(s, `${s.status === 'draft' ? '✎ ' : ''}${s.options.emoji ? `${s.options.emoji} ` : ''}${s.kind === 'post' ? (s.subject ? s.subject.slice(0, 40) : 'A post') : s.label}`, s.status === 'draft' ? `draft · ${KIND_WORD[s.kind] ?? s.kind} · tap to edit or ship` : sub, s.cents, s.stage, s.kind, false)}</button> : row(s, `${s.options.emoji ? `${s.options.emoji} ` : ''}${s.kind === 'post' ? 'A post' : s.label}`, sub, s.cents, s.stage, s.kind, mode === 'plan' && s.status === 'planned' && s.kind !== 'post')}
            ...
            {state !== 'done' && mode === 'campaigns' && <button type="button" onClick={() => { setPencil({ date: day, kind: 'post', subject: '' }); setDay(null) }} style={{ ...cta, marginTop: 14, justifyContent: 'center', gap: 8, background: '#fff', color: C.ink, border: `0.5px solid ${C.line}` }}>✎ Pencil in an idea for {niceDate(day)}</button>}
            {state !== 'done' && <button type="button" onClick={() => { setAdding(day); setDay(null) }} style={{ ...cta, marginTop: mode === 'campaigns' ? 8 : 14, justifyContent: 'center', gap: 8 }}><Plus size={16} /> Add something on {niceDate(day)}</button>}
          </div>
        </div>) })()}
      ...
      {piece && (() => { const x = piece; const isDraft = x.status === 'draft'; const extra = x.rate === 'oneoff'; const lk = !isDraft && locked(x); const fields = OPTIONS_OF[x.kind] ?? []; const opts = x.options ?? {}
        ...
        <div role="dialog" aria-modal="true" onClick={() => setPiece(null)} style={sheet}>
          <div className="cr pm-in" onClick={(e) => e.stopPropagation()} style={sheetIn}>
            ...
            {isDraft && <div style={{ fontSize: 11.5, color: C.mute, marginTop: 6, textAlign: 'center' }}>{x.date ? (sameDaySlot?.fill === 'open' ? `Ships into your open ${KIND_WORD[x.kind]} slot that day, free.` : `No open ${KIND_WORD[x.kind]} slot that day: ships as an extra, approved before it is charged.`) : 'Put it on a day to ship it.'}</div>}
            {!lk && x.status !== 'done' && <button type="button" disabled={busy != null} onClick={() => { if (isDraft || extra) move({ action: 'drop', key: x.id }, 'drop'); else move({ action: 'scrap', slotId: x.id }, 'scrap') }} style={{ ...cta, marginTop: 14, justifyContent: 'center', background: '#fff', color: '#c92d32', border: `0.5px solid ${C.line}`, height: 44, fontSize: 14 }}>{isDraft ? 'Throw the idea away' : extra ? 'Take it off' : 'Scrap the idea, keep the slot'}</button>}
          </div>
        </div>) })()}
      ...
      {pencil && (
        <div role="dialog" aria-modal="true" onClick={() => setPencil(null)} style={sheet}>
          <div className="cr pm-in" onClick={(e) => e.stopPropagation()} style={sheetIn}>
            ...
            <button type="button" disabled={busy != null || !pencil.subject.trim()} onClick={() => move({ action: 'draft', kind: pencil.kind, subject: pencil.subject, date: pencil.date }, 'pencil')} style={{ ...cta, marginTop: 12, justifyContent: 'center', gap: 8, opacity: pencil.subject.trim() ? 1 : .5 }}>{busy === 'pencil' ? <Loader2 size={16} className="mvp-spin" /> : '✎'} Keep it as a draft</button>
          </div>
        </div>
      )}
