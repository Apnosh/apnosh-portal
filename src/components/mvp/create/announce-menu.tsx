...
export function itemCents(it: ItemPick, p: MenuPrices, profile?: CreatorProfile | null): number {
  ...
  switch (it.id) {
    ...
    case 'graphic': { const where = (o.where as string[] | undefined) ?? ['post']; const n = Math.max(1, Math.min(6, Number(o.count) || 1)); if (o.from === 'shoot') return Math.max(0, n - (Number(o.included) || 0)) * PACKAGE_EXTRA.graphic; return Math.round(n * (p.graphic + (where.includes('tent') ? 2500 : 0) + (where.includes('poster') ? 2500 : 0) + (o.spanish ? 4000 : 0)) * (n >= 2 ? 0.9 : 1)) }
    case 'video': { if (o.filmed === 'shoot') return Math.max(0, Math.max(1, Math.min(6, Number(o.count) || 1)) - (Number(o.included) || 0)) * PACKAGE_EXTRA.video; const n = Math.max(1, Math.min(3, Number(o.count) || 1)); const each = p.video + (o.style === 'chef' ? 7500 : 0) + (o.tiktok ? 6000 : 0) + (o.spanish ? 2500 : 0); return Math.round(each * n * (n >= 2 ? 0.9 : 1)) + (o.filmed === 'visit' ? 15000 : 0) }
