...
export default function ReplySheet({ clientId, onClose }: { clientId: string; onClose: () => void }) {
  ...
  const Box = ({ value, onChange, onSend, onAgain, busy, writing, rows = 4, placeholder }: { value: string; onChange: (v: string) => void; onSend: () => void; onAgain: () => void; busy: boolean; writing: boolean; rows?: number; placeholder?: string }) => (
    <div style={{ position: 'relative', marginTop: 6 }}>
      <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={rows} placeholder={writing ? 'Writing it in your voice' : placeholder ?? 'Write a reply, or tap the circle to have it written'} ref={(el) => { if (!el) return; el.style.height = 'auto'; el.style.height = `${Math.max(el.scrollHeight, rows * 20 + 60)}px` }} style={{ ...ta, marginTop: 0, paddingBottom: 48, overflow: 'hidden' }} />
