...
export interface HomeFunnelProps {
  ...
  loading?: boolean
  /** the accounts on each ring (owner 2026-09-22): the connected accounts that feed each stage, as brand marks on the circle */
  marks?: string[][] | null
}
...
export default function HomeFunnel({
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      /* THE NAV'S REAL TOP EDGE (owner 2026-09-14): on a phone with a home bar the nav floats
         above the safe area, so its top is higher than "height from the bottom" assumed and
         the last ring stopped 30px short. Measured, the canvas ends exactly at the nav's edge. */
      const navTop = nav ? nav.getBoundingClientRect().top : vh - 78
      const navH = vh - navTop
  ...
  const layout = useMemo(() => {
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      /* the canvas ends AT the nav's top edge (measured above), so the last ring's edge sits 8px
         above the nav (owner 2026-09-14: as close as it can be without touching) */
      yBot = effH - 8 - RTOP * (n === 5 ? RRATIO[4] : 0.45)
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative', height: effH, background: C.funnelBg, overflow: 'hidden', fontFamily: "'Inter',system-ui,sans-serif", color: C.ink, ...(fill ? { border: 'none', borderRadius: 0, boxShadow: 'none' } : { border: `0.5px solid ${C.line}`, borderRadius: 20, boxShadow: '0 24px 60px -24px rgba(18,80,58,.30), 0 6px 18px -6px rgba(0,0,0,.06)' }) }}>
      ...
      <div style={bar ? { display: 'grid', gridTemplateColumns: '40px minmax(0, 1fr) 40px', alignItems: 'center', gap: 10, padding: '10px 12px 4px' } : { display: 'flex', alignItems: 'center', gap: 8, padding: '14px 16px 6px' }}>
        {bar && (
          <Link href="/dashboard/more" aria-label={T('Your business')} style={{ flexShrink: 0, width: 40, height: 40, borderRadius: '50%', padding: 2, background: 'linear-gradient(135deg, #4abd98 0%, #8ee5c6 45%, #ffd58a 100%)', boxShadow: theme === 'dark' ? '0 0 0 1px rgba(255,255,255,0.08)' : '0 1px 2px rgba(0,0,0,.04), 0 6px 18px rgba(46,154,120,.18)', textDecoration: 'none', display: 'block', boxSizing: 'border-box' }}>
      ...
      {curRange === 'custom' && onCStart && onCEnd && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '2px 16px 6px' }}>
          ...
          />
          <span style={{ fontSize: 12, color: C.faint, flexShrink: 0 }}>{T('to')}</span>
          <input
            type="date"
            value={cEnd ?? ''}
      ...
      />
      {/* THE ACCOUNTS ON EACH RING (owner 2026-09-22, "show what connected accounts are part of it"):
          the brand marks of the accounts feeding each stage, overlapping on the ring's outer rim. */}
      {marks && !loading && (
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 2 }}>
          {layout.map((L, i) => {
            const brands = marks[i] ?? []
 ...
 */
export function HomeFunnelSkeleton({ height = 620, message }: { height?: number; message?: string }) {
  const { C } = useMvpTheme()
  const { T } = useLang()
  const line = message ?? T('Getting your numbers')
  ...
  return (
    <div style={{ height, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 0 0', boxSizing: 'border-box', overflow: 'hidden' }} aria-busy aria-label={line}>
      <style>{`
...
@media (prefers-reduced-motion: reduce){.hf-ghost-sweep{display:none}.hf-ghost-ring{animation:none;opacity:.8}}`}</style>
      ...
      <div style={{ fontSize: 14, fontWeight: 500, color: C.mute, padding: '6px 0 14px', letterSpacing: '.01em' }}>{line}</div>
    </div>
  )
}
...
type FunnelData = { providers?: string[][] | null; views: Views | null; actions: Actions | null; counts: StageCounts | undefined; asOf: string | null; windowStart: string | null; windowEnd: string | null; audience: string | null; yoy: FunnelYoY | null; yoyAbs: FunnelYoYAbs | null }
...
export function HomeFunnelLive({ clientId, height, fill, fillReserve, onVisibility, bar, tickFor }: { clientId?: string; height?: number; fill?: boolean; /** px the rows below the hero need; passed straight through to HomeFunnel's fill maths */ fillReserve?: number; onVisibility?: (v: 'shown' | 'empty') => void; bar?: HomeFunnelProps['bar']; /* the +/- ticks from the SAME daily series Insights charts (2026-09-04), so the two screens agree by construction; a null field keeps the route's own read */ tickFor?: (r: FunnelRange) => Partial<FunnelYoY> | null }) {
  ...
  const marks = data?.providers ?? null
  ...
  useEffect(() => {
    ...
    const load = () => fetch(`/api/dashboard/insights-detail?clientId=${clientId}&range=${range}${custom}&today=${localYmd()}`)
      ...
      .then((d) => {
        ...
        setData({
          views, actions,
          providers: Array.isArray(d.providers) ? (d.providers as string[][]) : null,
