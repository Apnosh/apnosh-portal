...
/* No red on Insights (owner 2026-09-13): a stage wears its own bright colour whatever the period did */
const verdictColor = (stageKey: string): string => ((STAGE_BRIGHT[stageKey] ?? TREND_GREEN))
...
function StageWithChart({ mv, label, cs, unit, breakdownTitle, clientId, stageNumber, showBreakdown = true, onRange, onDir, accent }: { mv: MetricView; label: string; cs: ComputedStage | undefined; unit: string; accent?: string; /** reports the verdict colour up, so the by-source bars under the graph can match it */ onDir?: (hex: string) => void; breakdownTitle?: string; clientId?: string; stageNumber?: number; showBreakdown?: boolean; onRange?: (r: string) => void }) {
  ...
  const stars = mv.ratingDaily ? starsForBars(summary.bars, mv.ratingDaily) : undefined
  const b0 = summary.bars[0], bN = summary.bars[summary.bars.length - 1]
  const avgStars = mv.ratingDaily && b0?.sMs != null && bN?.sMs != null ? starsOver(mv.ratingDaily, b0.sMs, bN.eMs ?? bN.sMs) : null
    ...
    <>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, marginTop: 6, flexWrap: 'wrap' }}>
          ...
          {avgStars != null && (
            <span aria-label="Average rating in this range" style={{ display: 'inline-flex', alignItems: 'baseline', gap: 4, marginBottom: 3 }}>
              <span style={{ fontFamily: DISPLAY, fontSize: 22, fontWeight: 600, color: STAR, letterSpacing: '-.01em', lineHeight: 1 }}>{avgStars.toFixed(1)}★</span>
              <span style={{ fontSize: 12, color: C.mute }}>average</span>
            </span>
          )}
      ...
      <ActionsChart range={range} setRange={setRange} cStart={cStart} setCStart={setCStart} cEnd={cEnd} setCEnd={setCEnd} summary={summary} noun={mv.unit} showTotal={false} accent={ac} stars={stars} />
...
function StageTrendRow({ label, accent, mv, sm, locked, days, campaigns, on, first, onPick, cs, stageNumber, clientId, range, smooth = 7, customStart, customEnd }: { smooth?: number; customStart?: string; customEnd?: string; label: string; accent: Accent; mv?: MetricView; sm: ReturnType<typeof bucketsFor> | null; launches: number; locked: boolean; days: number; campaigns: StageCampaign[]; on: boolean; first: boolean; onPick: () => void; cs?: ComputedStage; stageNumber: number; clientId?: string; range: string }) {
  ...
  /* reputation: the average stars over the same window, under the review count (owner 2026-09-15) */
  ...
  return (
    <button type="button" onClick={onPick} aria-pressed={on} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '9px 8px', margin: '0 -8px', boxSizing: 'content-box', background: on ? accent.soft : 'none', borderRadius: on ? 12 : 0, border: 'none', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit', transition: 'background .15s' }}>
      ...
      <span style={{ textAlign: 'right', flexShrink: 0, minWidth: 64 }}>
        <span style={{ display: 'block', fontFamily: DISPLAY, fontSize: 16, fontWeight: 600, color: locked ? C.faint : C.ink, letterSpacing: '-.01em' }}>{locked ? '0' : total != null ? total.toLocaleString() : DASH}{rowStars != null && !locked && <span style={{ color: STAR, fontSize: 13, marginLeft: 5 }}>{rowStars.toFixed(1)}★</span>}</span>
...
function CampaignTrend({ mv, list, reviews = [], chartRange = '30d', title = 'Trend', onPins, litPin, footer, underGraph, customStart, customEnd, smooth = 7, note }: { mv?: MetricView; /** replaces the direction caption (a stage nothing feeds yet) */ note?: string; /** drawn right under the legend, before the campaigns (the Trends range row) */ underGraph?: React.ReactNode; list: StageCampaign[] | null; /** dated reviews, drawn under the same axis */ reviews?: InsightsReview[]; /** the stage chart's picked range — the trend follows it */ chartRange?: string; /** the custom window's edges (YYYY-MM-DD) when chartRange is 'custom' */ customStart?: string; customEnd?: string; /** the rolling window in days (1 = each day) */ smooth?: number; title?: string; /** which campaign got which pin number, for the list under the chart */ onPins?: (m: Record<string, number>) => void; /** the pin a tapped campaign row belongs to */ litPin?: number | null; /** the campaigns legend, rendered inside this card so it lines up with the pins */ footer?: React.ReactNode }) {
  ...
  const starRoll: (number | null)[] = starByDay ? days.map((_, i) => { let s = 0, k = 0; for (let j = Math.max(0, i - (smooth - 1)); j <= i; j++) { const d = starByDay.get(days[j].t); if (d) { s += d.sum; k += d.n } } return k > 0 ? s / k : null }) : []
  const starPts = starRoll.map((v, i) => (v == null ? null : { x: xOf(i), y: yStar(v), v })).filter((p): p is { x: number; y: number; v: number } => p != null)
  ...
  return (
    <div style={CARD}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        {avgStars != null && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 13.5, fontWeight: 600, color: STAR }}>{avgStars.toFixed(1)}★ <span style={{ color: C.mute, fontWeight: 500 }}>average</span></span>
        )}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', touchAction: 'pan-y' }} role="img" aria-label="Stage trend: every day, the 7-day average, the prior period, and campaign go-live markers"
        ...
        {starPts.length > 0 && (
          <g>
            {starPts.length > 1 && <polyline points={starPts.map((p) => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ')} fill="none" stroke={STAR} strokeWidth={1.8} strokeLinejoin="round" strokeLinecap="round" />}
            {starPts.map((p, i) => <circle key={`st${i}`} cx={p.x} cy={p.y} r={2.4} fill={STAR} stroke="#fff" strokeWidth={1} />)}
      ...
      <div style={{ minHeight: 22, marginTop: 8, fontSize: 11.5, display: 'flex', alignItems: 'center' }}>
        {pick != null ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: C.mute, width: '100%', whiteSpace: 'nowrap', overflow: 'hidden' }}>
            ...
            {starRoll[pick] != null && <b style={{ color: STAR }}>{starRoll[pick]!.toFixed(1)}★</b>}
          </div>
        ) : null}
      </div>
      {underGraph}
    ...
    <div style={LIST}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 6, alignItems: 'end', padding: '0 2px' }}>
        {byDay.map((d) => {
          ...
          const col = strong ? A.main : C.line
          return (
            <div key={d.day} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
              <span style={{ fontSize: 10.5, fontWeight: 700, color: strong ? A.main : C.faint, fontVariantNumeric: 'tabular-nums' }}>{d.avg.toLocaleString()}</span>
              <span aria-hidden style={{ width: '100%', height: Math.max(3, Math.round((d.avg / Math.max(1, max)) * 54)), background: col, borderRadius: 5 }} />
              <span style={{ fontSize: 11, fontWeight: strong ? 700 : 500, color: strong ? A.main : C.mute }}>{d.day}</span>
            </div>
          )
        })}
      </div>
 ...
 */
/**
 * ONE TILE, PORTRAIT, EDGE TO EDGE.
 * =================================
