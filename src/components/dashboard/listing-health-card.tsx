...
import { getListingHealth, type ListingHealth } from '@/lib/listing-health'
...
export default function ListingHealthCard({ clientId, locationId }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    getListingHealth(clientId, locationId ?? null)
  ...
  const ringColor = data.status === 'great'
    ? 'text-emerald-600'
    : data.status === 'good' ? 'text-amber-500' : 'text-rose-500'
  const headline = data.status === 'great'
    ? 'Your listing is in great shape'
    : data.status === 'good'
      ? 'Your listing is solid with room to improve'
      : 'Your listing needs work'
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="text-sm font-semibold text-ink">Listing health</h2>
          </div>
