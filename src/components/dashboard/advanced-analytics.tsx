...
export function AdvancedAnalytics({ metrics }: { metrics: AdvMetric[] }) {
  ...
  return (
    <div className="adv">
      ...
      {/* 1. Source breakdown over time — stacked bars */}
      <div className="adv-card">
        <div className="adv-card-h">
          <span className="adv-card-t">Where it came from</span>
          <span className="adv-card-s">stacked by source</span>
        </div>
        <div className="adv-chart">
          {view.colTotals.map((ct, i) => {
            if (ct == null) return <div key={i} className="adv-col"><div className="adv-seg-blank" /></div>
            return (
              <div key={i} className="adv-col">
                {view.active.map((s) => {
                  const v = s.vals[i] ?? 0
                  const h = (v / view.max) * 100
                  if (v <= 0) return null
                  const color = view.cards.find(c => c.key === s.key)?.color ?? PALETTE[0]
                  return <div key={s.key} className="adv-seg-fill" style={{ height: `${h.toFixed(2)}%`, background: color }} />
                })}
              </div>
            )
          })}
        ...
        {/* Legend — connected sources only; tap to toggle in/out */}
