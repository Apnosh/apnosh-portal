...
function LocationPicker() {
  ...
  const [currentlyLinkedName, setCurrentlyLinkedName] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    /* Find the location that's currently linked so we can mark it
       in the list. Best-effort — falls back gracefully if it fails. */
    fetch('/api/dashboard/gbp/status')
      .then(r => r.ok ? r.json() : null)
      .then(json => {
        if (json?.locationName) setCurrentlyLinkedName(json.locationName)
      })
      .catch(() => { /* ignore */ })
  }, [clientId])
    ...
    <div className="max-w-lg mx-auto px-6 py-12">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-ink-3">
          ...
          {currentlyLinkedName && <> Currently linked: <strong className="text-ink-2">{currentlyLinkedName}</strong>.</>}
        </p>
      </div>
          ...
          g.locations.length > 0 && (
            <div key={g.account.name}>
              ...
              <div className="space-y-2">
                {g.locations.map((loc) => {
                  const isLinked = currentlyLinkedName === loc.title
                  return (
                    <button
                      key={loc.name}
                      onClick={() => handlePick(g.account.name, loc)}
                      disabled={saving !== null}
                      className={`w-full text-left p-4 rounded-xl border transition-colors disabled:opacity-50 flex items-center justify-between group ${
                        isLinked
                          ? 'bg-brand-tint/40 border-brand/50'
                          : 'bg-white border-ink-6 hover:border-brand hover:bg-brand-tint/30'
                      }`}
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <p className={`text-sm font-semibold ${isLinked ? 'text-brand-dark' : 'text-ink group-hover:text-brand-dark'}`}>
                      ...
                      {saving === loc.name ? (
                        ...
                        <Check className={`w-4 h-4 ${isLinked ? 'text-brand-dark' : 'text-ink-5 group-hover:text-brand'}`} />
                      )}
                    </button>
                  )
                })}
