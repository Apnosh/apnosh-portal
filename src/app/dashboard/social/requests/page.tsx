...
import {
  Plus, ChevronRight, ListTodo, Search, ArrowLeft,
} from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import { useRealtimeRefresh } from '@/lib/realtime'
import type { ContentQueueItem, QueueStatus, ContentFormat } from '@/types/database'
...
export default function SocialRequestsPage() {
  ...
  const filtered = requests.filter(r => {
    ...
    if (formatFilter !== 'all' && r.content_format !== formatFilter) return false
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="font-[family-name:var(--font-display)] text-2xl text-ink">Social Media Requests</h1>
            <p className="text-ink-3 text-sm mt-1">Every social post request, active and archived.</p>
          </div>
        </div>
        <Link
      ...
      {noClient && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
          <p className="text-sm text-amber-900 font-medium">No content service linked yet</p>
          <p className="text-xs text-amber-700 mt-1">
            Contact your Apnosh team to enable content requests for your account.
          </p>
        </div>
      )}
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {Object.entries(FORMAT_LABEL).map(([val, label]) => (
                <option key={val} value={val}>{label}</option>
              ))}
            </select>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 text-[10px] text-ink-4">
                      {req.content_format && FORMAT_LABEL[req.content_format] && (
                        <span>{FORMAT_LABEL[req.content_format]}</span>
                      )}
