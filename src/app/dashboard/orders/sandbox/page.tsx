  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 flex items-center gap-3">
        <span className="bg-amber-400 text-amber-900 text-[10px] font-bold px-2 py-0.5 rounded-full tracking-wider">SANDBOX</span>
        <span className="text-amber-800 text-sm">Design playground. Changes here don't affect the live page.</span>
        <Link href="/dashboard/orders" className="ml-auto text-xs text-amber-600 hover:text-amber-800 underline">Back to live</Link>
      </div>
      ...
      {view === 'build' && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 items-start">
            ...
            {/* RIGHT: Customer Journey Funnel + Calculator (sticky) */}
            <div className="hidden lg:block sticky top-20 space-y-4">
              {/* ── Customer Journey Funnel ── */}
              {(() => {
                const selectedKeys = Object.keys(selected)
                const activeSvcIds = new Set(
                  ['social-media','websites','hosting-care','local-seo','email-sms','ai-automations','branding','photo-video','design-copy','strategy']
                    .filter(svcId => selectedKeys.some(k => k.startsWith(svcId)))
                )
                const funnelSteps = [
                  { id: 'local-seo', icon: '🔍', label: 'Found on Google', desc: 'People search, your business shows up', tab: 2, nudge: 'Most customers start with a Google search' },
                  { id: 'websites', icon: '🌐', label: 'Visit your website', desc: 'They click through and learn about you', tab: 1, nudge: 'No website = they go to a competitor' },
                  { id: 'social-media', icon: '📱', label: 'Follow on social', desc: 'They check your vibe and hit follow', tab: 0, nudge: '78% of diners check social before visiting' },
                  { id: 'email-sms', icon: '✉️', label: 'Get your emails', desc: 'Stay top of mind between visits', tab: 3, nudge: 'Email drives 2x more repeat visits than social' },
                  { id: 'ai-automations', icon: '🤖', label: 'Auto follow-up', desc: 'Never lose a lead or forget a review', tab: 4, nudge: 'Respond to reviews in minutes, not days' },
                ]
                const activeCount = funnelSteps.filter(s => activeSvcIds.has(s.id) || activeSvcIds.has('hosting-care') && s.id === 'websites').length
                const strength = Math.round((activeCount / funnelSteps.length) * 100)

                return (
                  <div className="rounded-3xl bg-white/55 backdrop-blur-xl border border-white/70 shadow-lg p-5">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="font-[family-name:var(--font-display)] text-sm text-ink">How customers find you</h3>
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${strength >= 80 ? 'bg-emerald-100 text-emerald-700' : strength >= 40 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600'}`}>
                        {strength}% covered
                      </span>
                    </div>
                    <p className="text-[11px] text-ink-4 mb-4">Each step is a chance to win or lose a customer.</p>

                    <div className="relative">
                      {/* Vertical connector line */}
                      <div className="absolute left-[19px] top-3 bottom-3 w-[2px] bg-ink-6/40" />
                      {/* Active line overlay */}
                      {activeCount > 0 && (
                        <div
                          className="absolute left-[19px] top-3 w-[2px] bg-gradient-to-b from-emerald-400 to-emerald-300 transition-all duration-700 ease-out"
                          style={{ height: `${(Math.max(0, activeCount - 1) / (funnelSteps.length - 1)) * 100}%` }}
                        />
                      )}

                      <div className="relative space-y-1">
                        {funnelSteps.map((step, i) => {
                          const isActive = activeSvcIds.has(step.id) || (step.id === 'websites' && activeSvcIds.has('hosting-care'))
                          return (
                            <button
                              key={step.id}
                              onClick={() => setActiveTab(step.tab)}
                              className={`group w-full text-left flex items-start gap-3 p-2 rounded-xl transition-all duration-300 ${
                                isActive
                                  ? 'bg-emerald-50/80'
                                  : 'hover:bg-white/60'
                              }`}
                            >
                              {/* Node */}
                              <div className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center text-base shrink-0 transition-all duration-500 ${
                                isActive
                                  ? 'bg-emerald-100 shadow-md shadow-emerald-200/50 scale-100'
                                  : 'bg-white border-2 border-dashed border-ink-6/50 scale-90 opacity-60 group-hover:opacity-80 group-hover:scale-95'
                              }`}>
                                {isActive ? <span>{step.icon}</span> : <span className="grayscale">{step.icon}</span>}
                              </div>

                              {/* Text */}
                              <div className="flex-1 min-w-0 pt-0.5">
                                <div className={`text-sm font-medium transition-colors ${isActive ? 'text-ink' : 'text-ink-4'}`}>
                                  {step.label}
                                  {isActive && <CheckCircle2 className="inline w-3.5 h-3.5 ml-1.5 text-emerald-500" />}
                                </div>
                                <div className={`text-[11px] leading-tight mt-0.5 ${isActive ? 'text-ink-3' : 'text-ink-5'}`}>
                                  {isActive ? step.desc : step.nudge}
                                </div>
                              </div>

                              {/* Add prompt for inactive */}
                              {!isActive && (
                                <span className="text-[10px] text-brand-dark font-medium opacity-0 group-hover:opacity-100 transition-opacity pt-1 shrink-0">
                                  + Add
                                </span>
                              )}
                            </button>
                        ...
                        {/* Final destination: always active */}
                        <div className="flex items-start gap-3 p-2">
                          <div className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center text-base shrink-0 transition-all duration-500 ${
                            activeCount >= 3
                              ? 'bg-brand-tint shadow-md shadow-brand/20 scale-100'
                              : 'bg-white border-2 border-dashed border-ink-6/50 scale-90 opacity-40'
                          }`}>
                            🚪
                          </div>
                          <div className="flex-1 pt-0.5">
                            <div className={`text-sm font-medium transition-colors ${activeCount >= 3 ? 'text-ink' : 'text-ink-5'}`}>
                              Customer walks in
                            </div>
                            <div className={`text-[11px] mt-0.5 ${activeCount >= 3 ? 'text-brand-dark font-medium' : 'text-ink-5'}`}>
                              {activeCount >= 5 ? 'Every path leads to your door' :
                               activeCount >= 3 ? `${5 - activeCount} gap${5 - activeCount > 1 ? 's' : ''} left to fill` :
                               'Fill the gaps above to get here'}
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <div className="rounded-3xl bg-white/55 backdrop-blur-xl border border-white/70 shadow-lg p-6">
                <h3 className="font-[family-name:var(--font-display)] text-base text-ink mb-1">Your plan</h3>

                {entries.length === 0 ? (
                  <p className="text-sm text-ink-4 py-2">Pick services from the left to start building.</p>
                ) : (
                  <>
                    {/* Cost per day callout */}
                    {totalMonthly > 0 && (
                      <div className="bg-brand-tint/50 rounded-xl px-3 py-2 mb-3">
                        <span className="text-sm text-brand-dark font-semibold">${Math.round(totalMonthly / 30)}/day</span>
                        <span className="text-xs text-ink-4 ml-1.5">— less than one DoorDash fee</span>
                      </div>
                    )}
