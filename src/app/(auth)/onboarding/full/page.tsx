...
export default function OnboardingPage() {
  ...
  return (
    /* THE APP FRAME. One full-viewport column with three fixed zones (top bar,
       scrolling content, pinned action bar) shared with /preview/onboarding via
       OnboardingFrame, so what the owner previews is exactly what ships. */
    <OnboardingFrame
      step={screenNo}
      totalSteps={totalScreens}
      pct={showSuccess ? 100 : pct}
      onBack={goBack}
      showBack={!loading && !showSuccess && screenNo > 1}
      canSkip={!loading && !showSuccess && !!(data.role && data.biz_name && data.biz_type) && screenNo < totalScreens}
