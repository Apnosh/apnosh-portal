...
import {
  ArrowLeft, Loader2, Send, Upload, X, ImageIcon,
} from 'lucide-react'
...
export default function NewRequestPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  async function handleSubmit() {
    ...
    if (photoFile) {
      ...
      if (user) {
        ...
        const path = `${slug}/requests/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`
    ...
    if (result.success) {
      router.push(`/client/${slug}/requests/${result.data?.requestId}`)
    } else {
