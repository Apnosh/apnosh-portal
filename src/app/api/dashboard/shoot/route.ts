 ...
 *                              could join it
 *   POST { action: 'book', tier, date?, note? }
...
import { TIERS, TIER_LABEL, TIER_SMALL, SPOTS, tierCents, asTier, openShoot, shootSuggestions, bookShoot, attachToShoot } from '@/lib/shoot/day'
...
export async function GET(req: NextRequest) {
  ...
  const tiers = TIERS.map((t) => ({ id: t, label: TIER_LABEL[t], small: TIER_SMALL[t], spots: SPOTS[t], cents: tierCents(t) }))
...
export async function POST(req: NextRequest) {
  const body = (await req.json().catch(() => ({}))) as { clientId?: string; action?: string; tier?: string; date?: string; note?: string; shootId?: string; label?: string; kind?: string; planId?: string; pieces?: unknown; index?: number }
  ...
  if (body.action === 'book') {
    ...
    const r = await bookShoot(admin, { clientId, userId: access.userId, tier: asTier(body.tier), date, note: body.note })
