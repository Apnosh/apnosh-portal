...
import { templateMenuPromptBlock, composedLayoutPromptBlock } from '@/components/sites/registry'
...
For multi-variant mode: each variant MUST push apart on:
  - **Different identity.templateId** — pick a different one per variant from the available templates listed below. At least ONE variant should use "restaurant-composed" with a thoughtfully picked layout map. NEVER produce three variants with the same template.
...
For "restaurant-composed" variants, ALSO output a complete \`layout\` object choosing one variant per section from the section-variant catalogue. Mix the variants to produce a layout the templates can't — e.g. magazine hero + manifesto about + full-bleed-each locations + cinematic ayce, or sensory hero + centered-narrative about + list-typographic locations + table ayce.

NEVER produce three near-identical variants. Different layouts are non-negotiable.`.trim()
