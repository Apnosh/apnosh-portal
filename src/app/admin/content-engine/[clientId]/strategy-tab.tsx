...
export default function StrategyTab({
  ...
  return (
    <div className="space-y-5">
      ...
      {genError && <p className="text-sm text-red-600 bg-red-50 p-3 rounded-lg">{genError}</p>}
      <button
        onClick={handleGenerate}
        disabled={generating}
        className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-brand text-white text-sm font-semibold rounded-xl hover:bg-brand-dark transition-colors disabled:opacity-50"
      >
        {generating ? (
          <><Loader2 className="w-4 h-4 animate-spin" /> {genPhase || 'Generating...'}</>
        ) : (
